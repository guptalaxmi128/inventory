...
import { getEmployeeTicketById } from "../../../actions/employee/myTicket/myTicket";
...
const EmployeeTimeLine = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await dispatch(getEmployeeTicketById(ticketId));
        setData(res.data);
      } catch (error) {
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <Breadcrumb style={{ margin: "22px 0" }}>
          ...
          <Breadcrumb.Item>
            <a href="/employee/my-ticket">My Tickets</a>
          </Breadcrumb.Item>
